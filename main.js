...
(function () {
  ...
  let itemsOnPart = 10;
  let currentPart = 0;
  let totalUserItems = users.length;

  function processUser(id) {
    let random = (Math.floor(Math.random() * 10) + 1) * 100;
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        console.log('Waited: ', random, id);
        resolve(id);
      }, random);
    });
  }
  ...
  function identNextUsersPart() {
    counter.start = itemsOnPart * currentPart;
    counter.end = itemsOnPart + counter.start;
    currentPart++;
  }

  let printResult = (totalResult) => document.querySelector('.total_result').innerHTML = `Total result: ${totalResult}`;

  function setResult(res) {
    totalResult += sumResult(res);
  }

  function getUsers() {
    let promises = [];
    let partUsers = users.slice(counter.start, counter.end);
    for (let i = 0; i < itemsOnPart; i++) {
      promises.push(processUser(partUsers[i].id));
    }
